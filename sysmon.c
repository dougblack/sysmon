...
int init_module()
{
   ...
   head = (struct log_block *) kmalloc(sizeof(struct log_block), GFP_KERNEL);
   head->line_count = 0;
   head->next = NULL;
   head->id = 0;
   current_block = head;
...
{
   int i;
   struct log_block *block_ptr;
   struct log_block *temp;
   for (i = 0; i < 30; i++) {
      unregister_kprobe(&(probes[i]));
   }

   for (block_ptr = head; block_ptr != NULL;) {
      temp = block_ptr;
      printk(KERN_INFO MODULE_NAME "Freeing block %d.\n", temp->id); block_ptr = block_ptr->next;
      kfree(temp->lines);
      kfree(temp);
   }
